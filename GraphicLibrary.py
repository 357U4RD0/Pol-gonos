...
class Renderer:
    ...
    # DEBUG: verificar que los vértices tengan suficientes datos
        if len(A) < 9:
            print(f"ERROR: Vértice A tiene solo {len(A)} elementos: {A}")
            return
        if len(B) < 9:
            print(f"ERROR: Vértice B tiene solo {len(B)} elementos: {B}")
            return  
        if len(C) < 9:
            print(f"ERROR: Vértice C tiene solo {len(C)} elementos: {C}")
            return
